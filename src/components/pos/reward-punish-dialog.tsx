...
export function RewardPunishDialog({ open, onOpenChange, editId, lockedUser, onSaved }: Props) {
  ...
  useEffect(() => {
    ...
    fetchDetail({ url: REWARD_PUNISH_ENDPOINTS.detail, params: { id: editId } })
      ...
      .catch(e => toast.error(errMsg(e, t('components.rewardPunishDialog.requestFailed'))))
    // `lockedUser` is an inline object at the call site; re-running would wipe input.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open, editId, fetchDetail])

  const handleSave = async () => {
    if (!form.User?.Id) { toast.error(t('components.rewardPunishDialog.selectEmployeeError')); return }
    if (!form.RewardPunishReason?.Id) { toast.error(t('components.rewardPunishDialog.selectReasonError')); return }
    try {
      await request({
      ...
      toast.success(form.Id ? t('components.rewardPunishDialog.updateSuccess') : t('components.rewardPunishDialog.addSuccess'))
      onOpenChange(false)
      onSaved()
    } catch (e) { toast.error(errMsg(e, t('components.rewardPunishDialog.requestFailed'))) }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>{form.Id ? t('components.rewardPunishDialog.editTitle') : t('components.rewardPunishDialog.title')}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1">
              ...
              <Input value={form.Name ?? ''} disabled placeholder={t('components.rewardPunishDialog.autoGenerated')} />
            </div>
          </div>
          ...
          <div className="space-y-1">
            <Label>{t('components.rewardPunishDialog.type')}</Label>
            <div className="flex gap-2">
              {[
                { v: REWARD_PUNISH_TYPE.REWARD, label: t('components.rewardPunishDialog.reward') },
                { v: REWARD_PUNISH_TYPE.PUNISH, label: t('components.rewardPunishDialog.punish') },
              ].map(opt => (
                <button key={opt.v} type="button"
                  // Switching type invalidates the chosen reason — reasons are per type.
                  onClick={() => setForm(f => ({ ...f, Type: opt.v, RewardPunishReason: null }))}
                  className={`flex-1 rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${
                    form.Type === opt.v ? 'border-primary bg-primary/10 text-primary' : 'text-muted-foreground'}`}>
                  {opt.label}
                </button>
              ))}
            </div>
        ...
        <DialogFooter>
          ...
          <Button onClick={handleSave} disabled={saving}>{saving ? t('components.rewardPunishDialog.saving') : t('common.save')}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  )
