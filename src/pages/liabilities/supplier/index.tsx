...
export default function LiabilitySupplierPage() {
  const [keyword, setKeyword] = useState('')
  const [page, setPage] = useState(1)
  const [pageSize, setPageSize] = useState(PAGE_SIZE)
  const [dateFrom, setDateFrom] = useState(defaultDateFrom())
  const [dateTo, setDateTo] = useState(defaultDateTo())
  const [supplierGroup, setSupplierGroup] = useState<LookupItem | null>(null)
  ...
  const { data, isLoading, refetch } = useFilterReportQuery({
    path: 'liabilities/filter-supplier',
    params: {
      Keyword: keyword || undefined, DateFrom: dateFrom, DateTo: dateTo,
      SupplierGroupId: supplierGroup?.Id || undefined, IsShowAll: showAll,
  ...
  // pos_web raises this off PaymentDetailComponent but still tags it
  // RECEIPT_PAYMENT_FOR.LIABILITY_CUSTOMER (=1), not LIABILITY_SUPPLIER — kept
  // as-is to match how the server actually links these vouchers today.
  const openPay = (row: TPosDebtItem) => {
    const s = row.Supplier
    setPayFor({
      ReceiptPaymentType: RECEIPT_PAYMENT_FOR.LIABILITY_CUSTOMER,
      ObjectType: OBJECT_TYPE.Supplier,
      Supplier: s ?? null,
      ObjectName: s?.Name ?? row.ObjectName ?? '',
      Address: s?.Address ?? '',
      Amount: row.Total ?? 0,
    })
  }
  ...
  const columns: ColumnDef<TPosDebtItem>[] = [
    ...
    { id: 'code', header: 'Mã NCC', cell: ({ row }) => <span className="text-xs">{row.original.Supplier?.SupplierCode ?? '—'}</span> },
    {
      id: 'supplier', header: 'Nhà cung cấp',
      cell: ({ row }) => {
        const s = row.original.Supplier
        return <span className="font-medium">{s?.Name ?? row.original.ObjectName ?? '—'}</span>
      },
    },
    { id: 'group', header: 'Nhóm', cell: ({ row }) => <span className="text-xs">{row.original.Supplier?.SupplierGroup?.Name ?? '—'}</span> },
    { id: 'address', header: 'Địa chỉ', cell: ({ row }) => <span className="text-xs text-muted-foreground">{row.original.Supplier?.Address ?? '—'}</span> },
    { id: 'phone', header: 'Điện thoại', cell: ({ row }) => <span className="text-xs">{row.original.Supplier?.Phone ?? '—'}</span> },
    {
  ...
  return (
    <div className="space-y-4">
      <ListPageHeader title="Công nợ nhà cung cấp" icon={Truck}>
        <SearchBar value={keyword} onChange={v => { setKeyword(v); setPage(1) }} placeholder="Tìm nhà cung cấp..." />
        <LookupSelect className="w-44" endpoint="suppliergroups/filter-simple" placeholder="Tất cả nhóm"
          value={supplierGroup} onChange={v => { setSupplierGroup(v); setPage(1) }} />
      ...
      <DataTable
        ...
        emptyText="Không có công nợ nhà cung cấp"
      />

      <LiabilityDetailDialog
        open={detailFor != null}
        onOpenChange={o => { if (!o) setDetailFor(null) }}
        endpoint="liabilities/filter-liabilities-supplier"
        supplierId={detailFor ?? undefined}
      />
