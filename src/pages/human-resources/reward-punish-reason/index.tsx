...
import { ListChecks, Plus, ToggleLeft, ToggleRight } from 'lucide-react'
...
interface TRPReason { Id?: number; Name?: string; Note?: string; Status?: { Id?: number; Name?: string } }
const EMPTY = (): TRPReason => ({ Name: '' })
...
export default function RewardPunishReasonPage() {
  ...
  const { mutate: save, isLoading: saving } = useApiMutation(
    (body: TRPReason) => ({ url: body.Id ? 'rewardpunishreasons/update' : 'rewardpunishreasons/create', method: 'POST' as const, body }),
  ...
  const { mutate: toggleStatus } = useApiMutation(
    ({ id, statusId }: { id: number; statusId: number }) => ({ url: `rewardpunishreasons/update-status?id=${id}&statusId=${statusId}`, method: 'POST' as const, body: {} }),
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable columns={columns} data={items} loading={isLoading} total={total} page={page} pageSize={PAGE_SIZE} onPageChange={setPage} emptyText="Không có lý do nào" />
